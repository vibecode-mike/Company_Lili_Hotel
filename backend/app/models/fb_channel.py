...
class FbChannel(Base):
    ...
    page_id = Column(String(255), unique=True, nullable=True, comment="Facebook Page ID")
    page_access_token = Column(String(500), nullable=True, comment="FB 頻道存取權杖")
    ...
    is_active = Column(
        Boolean, default=True, nullable=False, comment="是否啟用（管理員手動控制）"
    )
    ...
    updated_at = Column(
        DateTime, onupdate=func.now(), nullable=True, comment="更新時間"
    )
