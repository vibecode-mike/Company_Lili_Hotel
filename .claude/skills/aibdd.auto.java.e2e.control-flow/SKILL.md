---
name: aibdd.auto.java.e2e.control-flow
description: Java E2E 全自動批次迴圈。掃描 features 目錄，為每個 .feature 展開完整的 5 phase TODO 清單（schema-analysis → step-template → red → green → refactor），然後逐一執行直到全數完成。
user-invocable: true
...
| 任務 phase | 呼叫的 Skill |
|-----------|-------------|
| Schema Analysis | `/aibdd.auto.java.e2e.schema-analysis` |
| Step Template | `/aibdd.auto.java.e2e.step-template` |
| Red | `/aibdd.auto.java.e2e.red` |
| Green | `/aibdd.auto.java.e2e.green` |
| Refactor | `/aibdd.auto.java.e2e.refactor` |
