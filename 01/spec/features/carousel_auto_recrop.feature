Feature: 輪播卡片圖片自動重新裁切（前端實現）
  ...
  Rule: 圖片裁切比例根據卡片欄位自動判定
    ...
    # - 勾選任何其他欄位（標題/內文/金額/按鈕）→ 1.91:1 橫向長方形 (1920x1005px)
  ...
  Rule: 勾選欄位導致比例改變時，前端自動重新裁切
    ...
    Example: 勾選內文欄位也觸發比例改變
      ...
      Then 前端自動從 originalFile 重新裁切為 1.91:1
      And 預覽區更新為橫向長方形
  ...
  Rule: 取消勾選欄位導致比例改變時，前端自動重新裁切
    ...
    Example: 橫向自動改回正方形（1.91:1 → 1:1）
      Given 行銷人員已上傳圖片「hotel_room.jpg」
      And 已勾選「選擇圖片」和「標題」欄位
      And 當前預覽顯示為 1.91:1 橫向長方形（1920x1005）
      And 原始圖片已存儲在 originalFile
      When 行銷人員取消勾選「標題」欄位
      And 此時僅保留「選擇圖片」勾選
      Then 前端檢測到比例需求從 1.91:1 變更為 1:1
      And 前端從 originalFile 使用 Canvas API 重新裁切
  ...
  Rule: 多次切換欄位時，前端可重複重新裁切
    ...
    Example: 反覆切換欄位導致多次自動裁切
      ...
      When 行銷人員勾選「標題」（觸發裁切為 1.91:1）
      And 行銷人員取消「標題」（觸發裁切回 1:1）
      And 行銷人員勾選「內文」（觸發裁切為 1.91:1）
  ...
  Rule: 圖片上傳採全有全無，任一張失敗即阻擋建立

    Example: 上傳多張輪播圖片，任一張失敗則阻擋
      Given 行銷人員建立輪播訊息，需上傳 3 張圖片
      And 圖片 1 上傳成功
      And 圖片 2 上傳失敗（HTTP 500）
      And 圖片 3 上傳成功
      When 系統收到上傳結果
      Then 系統標記上傳失敗，阻擋建立或儲存
      And 前端顯示錯誤訊息「圖片 2 上傳失敗，請重試」
      And 後端不建立訊息記錄，也不儲存任何已上傳的圖片 URL
      And 使用者需修正後重新提交（不可局部成功）
  ...
  Rule: 圖片品質和效能保證
    ...
    Example: 裁切品質保證
      ...
      And 1.91:1 輸出尺寸固定為 1920x1005px
  ...
  Rule: 內存管理和資源清理
    ...
    Example: 自動重新裁切時清理舊的 Blob URL
      ...
      And 前端生成新的 1.91:1 裁切圖片的 Blob URL
      And 更新預覽並避免內存洩漏
