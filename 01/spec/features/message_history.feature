Feature: 訊息紀錄
  ...
  Rule: 完整記錄會員與內部人員的所有聊天內容
    ...
    Example: 完整記錄聊天內容
      ...
      Then 系統顯示以下訊息
        ...
        | user      | incoming  | 請問有雙人房嗎?             |
        | assistant | outgoing  | 有的，請問您想預訂哪一天?   |
    ...
    Example: 記錄包含圖片與貼圖的聊天內容
      ...
      When 使用者查看聊天記錄
      Then 系統完整顯示以下訊息
        | role      | direction | message_type | content      |
        | user      | incoming  | sticker      | 熊大比讚     |
        | assistant | outgoing  | image        | 房間照片.jpg |
        | user      | incoming  | text         | 看起來不錯   |
  ...
  Rule: 訊息即時更新
    ...
    # SSE 原生兼容 HTTP/2，前端使用 EventSource API（自帶重連機制）

    Example: 會員發送訊息後即時顯示於聊天介面
      Given 內部人員正在與會員「張三」聊天
      And 系統透過 SSE 建立即時連線（GET /api/v1/sse/chat/{thread_id}）
  ...
  Rule: SSE 斷線自動重連（EventSource 原生重連，預設約 3 秒），失敗後提示手動重試

    Example: 斷線後自動重連
      Given 內部人員正在與會員「張三」聊天
      And 系統透過 SSE 建立即時連線
      When SSE 連線因網路問題中斷
      Then EventSource 自動發起重連（瀏覽器原生機制，預設約 3 秒間隔）
      And 重連成功後自動恢復即時訊息流與送達狀態

    Example: 重連持續失敗後提示手動重試
      Given SSE 連線已多次重連皆失敗
      When 連線持續無法建立
      Then 聊天介面顯示提示訊息「連線已中斷，請檢查網路或手動重試」
      And 提供「重新連線」按鈕讓使用者手動觸發

    Example: 連線中斷期間發送訊息的處理
      Given 內部人員正在與會員「王五」聊天
      And SSE 連線已中斷且正在自動重連中
      When 內部人員嘗試發送訊息「請稍候」
      Then 系統暫停送出並顯示提示「連線恢復後自動送出」
      And 若最終重連失敗，顯示錯誤「連線已中斷，訊息未送出，請手動重試」
  ...
  Rule: 紀錄對話開啟日期，格式為 YYYY/MM/DD（中文星期）

    Example: 記錄對話開啟日期
      Given 會員「王五」於「2025/01/20（一）」開始對話
      When 使用者查看聊天記錄
      Then 系統顯示對話開啟日期「2025/01/20（一）」

    Example: 週末開始對話顯示正確的星期格式
      Given 會員「劉大明」於「2025/02/15（六）」開始對話
      When 使用者查看聊天記錄
      Then 系統顯示對話開啟日期「2025/02/15（六）」

    Example: 跨年度對話顯示正確日期格式
      Given 會員「陳小華」於「2024/12/31（二）」開始對話
      And 對話持續至「2025/01/02（四）」
      When 使用者查看聊天記錄
      Then 系統顯示對話開啟日期「2024/12/31（二）」
  ...
  Rule: 聊天頁面左側欄顯示完整會員資訊（可直接編輯）

    # 左側欄結構：
    # 1. 頭像（依渠道切換 LINE/FB/Webchat 頭像）+ 顯示名稱
    # 2. 會員資料卡（點擊進入編輯模式）：
    #    - 必填：姓名、居住地、手機號碼、Email
    #    - 選填：生日、生理性別（男性/女性/不透露）、身分證字號、護照號碼
    # 3. 標籤區：會員標籤 + 互動標籤（可編輯）
    # 4. 唯讀資訊：加入來源、建立時間、最近聊天時間、會員 ID
    # 5. 內部備註（可編輯文字區域）

    Example: 聊天頁面顯示會員完整資訊
      Given 使用者正在與會員「張二一」聊天
      When 使用者查看左側會員資訊面板
      Then 系統顯示會員「張二一」的頭像與顯示名稱
      And 系統顯示會員資料卡：姓名、生日、生理性別、居住地、手機號碼、Email、身分證字號、護照號碼
      And 系統顯示唯讀資訊：加入來源、建立時間、最近聊天時間、會員 ID

    Example: 聊天頁面直接編輯會員資料
      Given 使用者正在與會員「張二一」聊天
      When 使用者點擊會員資料卡進入編輯模式
      And 使用者修改手機號碼為「0912345678」
      And 使用者點擊「儲存變更」
      Then 系統透過 PUT /api/v1/members/{id} 更新會員資料
      And 資料卡回到唯讀模式並顯示更新後的手機號碼

    Example: 會員資訊不完整時顯示已填寫欄位
      Given 會員「陳建華」僅填寫姓名與性別
      And 未填寫生日與聯絡方式
      When 使用者查看會員「陳建華」的資訊面板
      Then 系統顯示已填寫的姓名與性別
      And 未填寫的欄位顯示為空
