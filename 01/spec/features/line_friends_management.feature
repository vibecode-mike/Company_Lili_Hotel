Feature: LINE 好友管理
  ...
  # 實作決策註記（2025-11-20）
  # ============================================
  # v1.0 使用 Member 表處理 LINE 好友資料
  # - 不使用獨立的 LineFriend 表
  # - LINE 資料直接儲存於 Member 表（line_uid, line_name, line_avatar）
  # - Webhook 處理器位於 line_app/app.py（FollowEvent, UnfollowEvent）
  #
  # 原因：降低系統複雜度，避免資料同步問題
  ...
  Background:
    ...
    And Webhook 已啟用（line_app/app.py）
