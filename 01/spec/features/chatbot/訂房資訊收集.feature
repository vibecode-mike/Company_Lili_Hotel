...
Feature: 訂房資訊收集
  ...
  Rule: 後置（狀態）- 所有欄位齊全後 missing_fields 為空
    ...
    Example: 民眾提供跨月日期區間
      ...
      When 民眾傳送「3月7號住到3月9號」
      Then 系統解析 checkin_date = "2026-03-07"
      And checkout_date = "2026-03-09"
      And missing_fields 不包含 "checkin_date" 或 "checkout_date"

  Rule: 民眾提供過去日期時系統回覆警告並要求重新輸入
    ; ✓ 警告重填（選項 B）：_coerce_dates_not_in_past 不自動修正，改為回覆錯誤訊息
  ...
  Rule: Session rotate 時 booking_context 全清
    ...
    Example: 達到 5 輪上限 rotate 後民眾需重新提供訂房資訊
      Given 民眾 session 已進行 5 輪對話
      And booking_context.checkin_date = "2026-03-10"
      And booking_context.rooms = [{ room_type_code:"DLX", room_count:1 }]
