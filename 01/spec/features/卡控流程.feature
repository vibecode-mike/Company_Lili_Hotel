...
功能: 卡控流程
  ...
  Rule: 未綁定官方帳號時，點選功能模組顯示提示訊息

    Example: 未綁定官方帳號時顯示卡控提示
      Given 管理員已登入系統
      And 管理員已完成 LINE OA 設定但未綁定官方帳號
      When 管理員點選任一功能模組頁面
      Then 操作失敗
      And 系統顯示文字提示「請先設定官方帳號，才可使用功能模組」

  Rule: 未獲得系統授權時無法使用功能

    Example: 授權狀態未啟用時阻擋模組存取
      Given 管理員「李經理」的授權狀態為「未啟用」
      When 管理員登入系統後點選「會員管理」模組
      Then 操作失敗
      And 系統阻擋該模組頁面載入
      And 系統提示需完成授權才可使用功能模組

    Example: 授權過期時立即強制登出
      Given 管理員「張經理」已登入系統，授權到期日為「2025/07/31」
      When 系統偵測當前時間為「2025/08/01 00:05」
      Then 系統將該管理員所有登入會話標記為失效
      And 前端導向登入頁面
