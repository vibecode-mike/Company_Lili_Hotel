...
- **回答**：選擇 C。主表僅保留近 90 天資料；每日 02:00 排程批次將 `created_at < current_date - 90` 的資料搬移至 `MessageDeliveryArchive`（INSERT … SELECT），完成後再從主表刪除，同步紀錄搬移筆數與耗時。
- **更新的規格檔**：`spec/erm.dbml`
- **變更內容**：
  1. `MessageDelivery` Note 新增具體排程時間（每日 02:00）、批次搬移流程與日誌要求。
  2. `MessageDeliveryArchive` Note 補充查詢用途（僅供匯出/歷史查詢）與不得回寫主表的原則。
- **業務影響**：
  - 主表規模維持可控，確保發送狀態查詢效能。
  - 歷史查詢與合規需求仍可透過 Archive 表達成，無需額外外部儲存。
  - 排程作業具備審計紀錄，可監控失敗並在必要時重跑。
