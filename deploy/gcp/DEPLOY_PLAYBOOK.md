...
GRANT SELECT, INSERT, UPDATE, DELETE, CREATE, ALTER, DROP, INDEX, REFERENCES,
      EXECUTE, CREATE ROUTINE, ALTER ROUTINE, TRIGGER
...
**Routine / Trigger 權限**（EXECUTE/CREATE ROUTINE/ALTER ROUTINE/TRIGGER）：
- 我們有 migration（如 `26d892fb5b82_baseline_create_line_friends_with_sync_.py`）會 DROP/CREATE
  stored procedure 和 trigger（`sp_sync_*` + 6 個雙向同步 trigger）。
- 沒給這 4 個權限 → alembic 會撞 `ER_PROCACCESS_DENIED (1370): alter routine command denied`。
  注意 MySQL 的 1370 在「沒權限」時就先擋下，不管 routine 是否存在，連 `DROP IF EXISTS` 也過不了。
- **2026-05-06 staging 補上的歷史教訓**：原 GRANT 漏掉這 4 個，CI 在 line_friends migration 紅 X。

**外加 my.cnf 設定**：`[mysqld]` 區段加 `log_bin_trust_function_creators = 1`，
否則 `lili_app`（無 SUPER）建 trigger 會撞 `ER_BINLOG_UNSAFE_ROUTINE (1419)`。
MySQL 8 預設 `binlog_format=ROW`，這個變數打開風險可忽略（沒 statement-based replica 的問題）。

```bash
sudo tee -a /etc/mysql/mysql.conf.d/mysqld.cnf > /dev/null <<'EOF'
# 允許非 SUPER user（lili_app）建立 stored procedure / trigger
