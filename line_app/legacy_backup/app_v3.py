...
def build_user_messages_from_payload(payload: dict, campaign_id: int, line_user_id: str) -> List[FlexMessage]:
    ...
    if flex_message_json:
        try:
            ...
            logging.info("Using Flex Message JSON from payload")

            # TODO: 在 Flex Message 中添加追蹤 URI
            # 這需要遍歷 Flex Message 結構，找到所有 action.uri 並替換為追蹤 URI
            # 暫時直接使用原始 Flex Message

            fc = FlexContainer.from_dict(flex_data)
            messages.append(FlexMessage(alt_text=title, contents=fc))
            return messages
        except Exception as e:
            logging.error(f"Failed to parse flex_message_json: {e}, falling back to legacy format")
            # 如果解析失敗，繼續使用舊格式
