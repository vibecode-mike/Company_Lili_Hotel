...
def build_user_messages_from_payload(payload: dict, campaign_id: int, line_user_id: str) -> List[FlexMessage]:
    ...
    只支援 Flex Message JSON 格式
    """
    title = payload.get("title") or "活動通知"
    messages = []

    # 檢查必要欄位
    if not payload.get("flex_message_json"):
        error_msg = "Missing flex_message_json in payload"
        logging.error(f"❌ {error_msg}")
        raise ValueError(error_msg)

    # 解析並構建 Flex Message
    try:
        flex_json = json.loads(payload["flex_message_json"])
        logging.info(f"📦 Using Flex Message JSON, type: {flex_json.get('type', 'unknown')}")
        logging.debug(f"📦 Flex Message content: {json.dumps(flex_json, ensure_ascii=False, indent=2)}")

        fc = FlexContainer.from_dict(flex_json)
        messages.append(FlexMessage(alt_text=title, contents=fc))

        logging.info(f"✅ Successfully built Flex Message for campaign {campaign_id}")
        return messages

    except json.JSONDecodeError as e:
        error_msg = f"Invalid JSON in flex_message_json: {e}"
        logging.error(f"❌ {error_msg}")
        raise ValueError(error_msg)
    except Exception as e:
        error_msg = f"Failed to build Flex Message: {e}"
        logging.error(f"❌ {error_msg}")
        raise
