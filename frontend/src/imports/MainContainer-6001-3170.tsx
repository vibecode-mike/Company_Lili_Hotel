...
function Container6({ member }: { member?: MemberData }) {
  ...
  const handleSave = async () => {
    try {
      // Simulate backend API call
      // await saveMemberInfo(member?.id, { realName, phone, email });
      setIsEditing(false);
      showToast('儲存成功', 'success');
    } catch (error) {
      showToast('儲存失敗', 'error');
...
function Container13({ member }: { member?: MemberData }) {
  return (
...
function Container25({ member }: { member?: MemberData }) {
  return (
...
function Container26({ member, onNavigate }: { member?: MemberData; onNavigate?: (page: string, params?: { memberId?: string }) => void }) {
  return (
...
function MainContent({ member, onNavigate }: { member?: MemberData; onNavigate?: (page: string, params?: { memberId?: string }) => void }) {
  return (
    <div className="relative shrink-0 w-full" data-name="Main Content">
      <div className="size-full">
        <div className="box-border content-stretch flex flex-col gap-[32px] items-start p-[40px] relative w-full">
          ...
          <Container26 member={member} onNavigate={onNavigate} />
...
export default function MainContainer({ 
  ...
  return (
    <div className="bg-slate-50 content-stretch flex flex-col items-start relative size-full" data-name="Main Container">
      ...
      <MainContent member={member} onNavigate={onNavigate} />
    </div>
  );
}