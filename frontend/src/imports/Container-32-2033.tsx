...
// Simplified card for image click type
function Frame7({ imageUrl }: { imageUrl?: string }) {
  if (imageUrl) {
    return (
      <div className="basis-0 content-stretch flex gap-[10px] grow items-center justify-center min-h-px min-w-px relative shrink-0 w-full">
        <img src={imageUrl} alt="預覽圖片" className="w-full h-full object-cover rounded-t-[15px]" />
      </div>
    );
  }
  return (
...
function ButtonFilledButton7({ button1 }: { button1?: string }) {
  return (
    <div className="bg-white relative rounded-[15px] shrink-0 w-full" data-name="Button/Filled Button">
      <div className="flex flex-row items-center justify-center size-full">
        <div className="box-border content-stretch flex items-center justify-center p-[15px] relative w-full">
          <p className="basis-0 font-['Noto_Sans_TC:Regular',_sans-serif] font-normal grow leading-[1.5] min-h-px min-w-px relative shrink-0 text-[#383838] text-[18px] text-center">{button1 || '動作按鈕'}</p>
        </div>
      </div>
    </div>
  );
...
function CardActions7({ button1 }: { button1?: string }) {
  return (
    <div className="relative shrink-0 w-full" data-name="Card Actions">
      <div className="flex flex-col justify-center size-full">
        <div className="box-border content-stretch flex flex-col gap-[6px] items-start justify-center p-[9px] relative w-full">
          <ButtonFilledButton7 button1={button1} />
        </div>
      </div>
    </div>
  );
...
function CardImage7({ imageUrl, button1, enableButton }: { imageUrl?: string; button1?: string; enableButton?: boolean }) {
  return (
    <div className="bg-[#edf0f8] h-[240px] relative rounded-[15px] shrink-0 w-full" data-name="Card Image">
      <div className="bg-clip-padding border-0 border-[transparent] border-solid box-border content-stretch flex flex-col h-[240px] items-center justify-center relative w-full">
        <Frame7 imageUrl={imageUrl} />
        {enableButton && <CardActions7 button1={button1} />}
      </div>
    </div>
  );
}

function Container7({ imageUrl, button1, enableButton }: { imageUrl?: string; button1?: string; enableButton?: boolean }) {
  return (
    <div className="relative shrink-0 w-[288px]" data-name="Container">
      <div className="bg-clip-padding border-0 border-[transparent] border-solid box-border content-stretch flex flex-col gap-[24px] items-start overflow-clip relative rounded-[inherit] w-[288px]">
        <CardImage7 imageUrl={imageUrl} button1={button1} enableButton={enableButton} />
      </div>
    </div>
  );
}
...
export default function TriggerImagePreview({ cardData, triggerImageUrl, templateType, enableButton1 }: TriggerImagePreviewProps) {
  return (
    <div className="bg-gradient-to-b from-[#a5d8ff] relative rounded-[20px] size-full to-[#d0ebff]" data-name="Container">
      <div className="size-full">
        <div className="box-border content-stretch flex gap-[20px] items-start overflow-clip pb-[24px] pl-[24px] pr-0 pt-[24px] relative size-full">
          <Container />
          {templateType === 'image_click' && enableButton1 ? (
            <Container7 imageUrl={cardData?.imageUrl} button1={cardData?.button1} enableButton={enableButton1} />
          ) : (
