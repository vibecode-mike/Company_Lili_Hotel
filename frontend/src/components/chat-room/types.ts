...
export interface ChatMessage {
  ...
  messageType?: string;  // 'text' | 'chat' | 'room_cards' | 'member_form' | 'booking_result' | 'flex' 等
  roomCards?: RoomCard[];  // messageType='room_cards' 時填入
  flexMessage?: string | Record<string, any>;  // messageType='flex'（群發）時填入，原始 flex JSON（dict 或字串）
